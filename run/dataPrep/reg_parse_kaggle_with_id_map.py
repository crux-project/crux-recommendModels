...
def k_nearest_neighbors(vectors, target, k, remove_ids):
    ...
    while nearest_neighbors_indices.size ==0:
        nearest_neighbors_indices = np.argsort(distances)[:k]
        for index in np.nditer(nearest_neighbors_indices):
            if int(index) in set(int(x) for x in remove_ids):
                nearest_neighbors_indices =np.delete(nearest_neighbors_indices, np.where(nearest_neighbors_indices==index))
                k+=1
...
def read_nodes(file_path, pre_defined_data_map =None, pre_defined_model_map=None):
...
###import the  data id_map and model id_map
pre_defined_data_map_path = os.path.join('../../data/kaggle/graph_split', 'data_map_fixed.txt')
pre_defined_data_map_file = open(pre_defined_data_map_path,'rb')
pre_defined_data_node_map = pickle.loads(pre_defined_data_map_file.read())
pre_defined_data_map_file.close()
pre_defined_model_map_path = os.path.join('../../data/kaggle/graph_split', 'model_map_fixed.txt')
pre_defined_model_node_map_file = open(pre_defined_model_map_path,'rb')
pre_defined_model_node_map = pickle.loads(pre_defined_model_node_map_file.read())
pre_defined_model_node_map_file.close()




train_n_datas, train_n_models, data_map, model_map, datas, models, data_embed, model_embed = read_nodes(input_file_path, pre_defined_data_node_map, pre_defined_model_node_map)
