...
##important note: the edges here the data_node id is the mapping after data_id.map, not considering the second_
def knn_selection(total_budget, data_embed, num_train_nodes, num_test_nodes,edges, model_embed, data_id, remove_ids):
    num_edges_to_probe = total_budget//num_test_nodes
    test_data_node = data_id
    #get the KNN neighbors of data_id by embedding
    target_embed = data_embed[test_data_node]
    train_embeds = np.zeros(shape=(1,target_embed.shape[0]))
    for i in range(num_train_nodes):
        train_data_embed = np.reshape(data_embed[i],(1,-1))
        train_embeds = np.concatenate((train_embeds, train_data_embed), axis=0)
    train_embeds=np.delete(train_embeds,0, axis=0)
    k_nearest = 3
    nearest_data_ids = k_nearest_neighbors(train_embeds,target_embed, k_nearest,remove_ids)
    #The nearest_data_ids must reside in the data_node_map.keys()
    train_split=get_train_edges(nearest_data_ids, edges, data_embed, model_embed,num_edges_to_probe)
    return train_split


def get_train_edges(nearest_data_ids, edges, data_embed, model_embed,num_edges_to_probe):
    #build a matrix as "data_id" | "model_id" | "performance" |"weight" | "score" ="performance" *"weight"
    edges[:,:2] = edges[:,:2].astype(int)
    indices = np.where(np.in1d(edges[:,0],nearest_data_ids))[0]
    perform_matrix=np.zeros(shape=(indices.shape[0],5))
    for row in range(indices.shape[0]):
        for col in range(5):
            i = indices[row]
            if col ==0:
                perform_matrix[row][col]=edges[i][0]
            elif col ==1:
                perform_matrix[row][col] = edges[i][1]
            elif col == 2:
                perform_matrix[row][col] = edges[i][2]
            elif col ==3:
                data_node_embeds = data_embed[edges[i][0]]
                norm_data = np.linalg.norm(data_node_embeds)
                normalized_data_embeds =data_node_embeds / norm_data
                model_node_embeds = model_embed[edges[i][1]]
                norm_model = np.linalg.norm(model_node_embeds)
                normalized_model_embeds = model_node_embeds / norm_model
                perform_matrix[row][col] = np.dot(normalized_data_embeds,normalized_model_embeds)
            elif col==4:
                perform_matrix[row][col] = perform_matrix[row][col-1]* perform_matrix[row][col-2]
    indices=np.argsort(perform_matrix[:,-1])[::-1]
    ranked_arr = perform_matrix[:,:3][indices][:num_edges_to_probe]
    return ranked_arr


def k_nearest_neighbors(vectors, target, k, remove_ids):
    distances = np.linalg.norm(vectors - target, axis=1)
    nearest_neighbors_indices =np.empty((0,))
    num_nearest_neighbor = k
    while nearest_neighbors_indices.size < num_nearest_neighbor:
        nearest_neighbors_indices = np.argsort(distances)[:k]
        # test which nearest_neighbors_indices collapse with remove_ids
        c = np.in1d(nearest_neighbors_indices, remove_ids)
        count = np.count_nonzero(c==0)
        step = np.count_nonzero(c)
        nearest_neighbors_indices = nearest_neighbors_indices[c == False]
        if count < num_nearest_neighbor:
            k =num_nearest_neighbor+step
...
def read_edges(file_path, data_map, model_map,metric,test_flag=False,threshold_score=None):
    edges =[]
    with open(file_path, 'r') as f:
        lines = f.read().strip().split('\n')
    ...
    for line in lines:
        ...
        if metric=='balanced_accuracy':
            ...
            ranking_metric = float(ranking_metric)
        if metric=='accuracy':
            ranking_metric = metric_list.split(',')[0].replace(' ', '').replace('[','').replace(']','')
            ranking_metric = float(ranking_metric)
    ...
    threshold_f1_score =threshold_score
    if metric == 'f1_score':
        if not test_flag:
...
random_state=5
random_state_2=5
#input_file_path = os.path.join('../../data/hugging_face/graph_split', 'node.txt')
#input_file_path = os.path.join('../../data/hugging_face_ratio_0.2/graph_split', 'node.txt')
input_file_path = os.path.join('../../data/hugging_face_dummy10/graph_split', 'node.txt')
train_n_datas, train_n_models, data_map, model_map, datas, models, data_embed, model_embed = read_nodes(input_file_path)
#input_edge_path = os.path.join('../../data/hugging_face/graph_split', 'edge.txt')
#input_edge_path = os.path.join('../../data/hugging_face_ratio_0.2/graph_split', 'edge_ratio_0.2.txt')
input_edge_path = os.path.join('../../data/hugging_face_dummy10/graph_split', 'edge.txt')
...
user_selected_metric = 'accuracy'

#test_edge_path = os.path.join('../../data/hugging_face/graph_split', 'new_edge.txt')
#test_edge_path = os.path.join('../../data/hugging_face_ratio_0.2/graph_split', 'new_edge.txt')
test_edge_path = os.path.join('../../data/hugging_face_dummy10/graph_split', '10_edge.txt')
#process the new_edge.txt to a csv file:
#edges = change_to_csv_test_edges(test_edge_path,user_selected_metric)
#test_node_path = os.path.join('../../data/hugging_face/graph_split', 'new_node.txt')
#test_node_path = os.path.join('../../data/hugging_face_ratio_0.2/graph_split', 'new_node.txt')
test_node_path = os.path.join('../../data/hugging_face_dummy10/graph_split', '10_node.txt')
...
test_cut_off_threshold =0.8
sampling_ratio = 0.7
##not pruning any edges
test_flag= False
threshold_score =0.7
treshold_process_start_time = time.time()
#use 0.6 as threshold to filter unnecessary regression scores
edges = read_edges(input_edge_path, data_map, model_map,user_selected_metric, test_flag,threshold_score)
threshold_time = time.time() - treshold_process_start_time
print('Treshold prunning Time: {:.3f}s'.format(threshold_time))
test_flag= True
...
'''
id_index = 0
updated_data_embed =dict()
last_train_data_id =0
##it looks like that we need to compute the updated_data embed firstly
for i in range(indices.shape[0]-1):
    if data_ids[i]<train_n_datas and data_ids[i]==id_index:
        ...
        if model_pairs_with_same_data_id.shape[0]<2:
            ...
            key = int(data_ids[i])
            updated_data_embed[key] = data_embed[key]
    elif data_ids[i]<train_n_datas and data_ids[i]> id_index:
        print("The gap exists, we need to re-map the dataset id")
        key = int(data_ids[i])
        value = int(id_index)
        id_index +=1
        # the data_embed also needs to be updated accordingly
        updated_data_embed[value] = data_embed[key]
        model_pairs_with_same_data_id = edges[indices[i]:indices[i + 1], :]
        if model_pairs_with_same_data_id.shape[0] < 2:
            print("there may generate unexpected errors,  data id is {}".format(data_ids[i]))
            id_index -= 1
        ...
        print("The gap exists, we need to re-map the dataset id")
        last_train_data_id = int(data_ids[i-1])
        key = int(data_ids[i])
        value = int(id_index)
        id_index += 1
        # the data_embed also needs to be updated accordingly
...
for i in range(indices.shape[0]-1):
    ...
    #this branch should never be iterate in, just for robustness purpose
        ...
        if model_pairs_with_same_data_id.shape[0] < 2:
            ...
            knn_process_phase1_start_time = time.time()
            train_split = knn_selection(total_budget, data_embed, train_n_datas, num_test_nodes,edges, model_embed, key, removed_ids)
            knn_prob_phase1 = time.time()-knn_process_phase1_start_time
            print('knn probing phase 1 Time: {:.3f}s'.format(knn_prob_phase1))
            knn_process_time.append(knn_prob_phase1)
            _, test_split = train_test_split(model_pairs_with_same_data_id, test_size=test_size_ratio,
...
if model_pairs_with_same_data_id.shape[0] >=2:
    ...
    num_test_nodes = n_datas - train_n_datas
    test_size_ratio = compt_ratio(total_budget, num_candidates, num_test_nodes)
    _, test_split = train_test_split(model_pairs_with_same_data_id, test_size=test_size_ratio, random_state=random_state_2)
    knn_process_phase2_start_time = time.time()
    train_split = knn_selection(total_budget, data_embed, train_n_datas, num_test_nodes, edges, model_embed, key,
                                removed_ids)
    knn_prob_phase2 = time.time() - knn_process_phase2_start_time
    print('knn probing phase 2 Time: {:.3f}s'.format(knn_prob_phase2))
    knn_process_time.append(knn_prob_phase2)
    train_split[:, 0] = value
    added_train_test_phase_edges += train_split.shape[0]
...
#this branch should never been interated
    ...
    removed_ids.append(data_ids[last_node_id])
    key = int(data_ids[last_node_id])
    value = int(id_index)
    id_index += 1
    second_level_data_map[key] = value
...
#assert(len(second_level_data_map)==41)

print("total probing time is {:.4f}".format(sum(knn_process_time)))
...
#train_ground_truth_file_path = os.path.join('../../data/hugging_face/output','train_ground_truth.csv')
#train_ground_truth_file_path = os.path.join('../../data/hugging_face_ratio_0.2/output','train_ground_truth.csv')
train_ground_truth_file_path = os.path.join('../../data/hugging_face_dummy10/output','train_ground_truth.csv')
...
#valid_ground_truth_file_path = os.path.join('../../data/hugging_face/output','valid_ground_truth.csv')
#valid_ground_truth_file_path = os.path.join('../../data/hugging_face_ratio_0.2/output','valid_ground_truth.csv')
valid_ground_truth_file_path = os.path.join('../../data/hugging_face_dummy10/output','valid_ground_truth.csv')
np.savetxt(valid_ground_truth_file_path,valid_ground_truth, fmt='%i,%i,%f',delimiter=',')

##save the second_level_data_map
#output_second_data_map_file_path = os.path.join('../../data/hugging_face/output','second_data_map.txt')
#output_second_data_map_file_path = os.path.join('../../data/hugging_face_ratio_0.2/output','second_data_map.txt')
output_second_data_map_file_path = os.path.join('../../data/hugging_face_dummy10/output','second_data_map.txt')
...
#output_train_file_path = os.path.join('../../data/hugging_face/output', 'inductive_train.txt')
#output_train_file_path = os.path.join('../../data/hugging_face_ratio_0.2/output', 'inductive_train.txt')
output_train_file_path = os.path.join('../../data/hugging_face_dummy10/output', 'inductive_train.txt')
...
for items in train_edge_list:
    ...
    f.write('\n')
f.close()
#output_val_file_path = os.path.join('../../data/hugging_face/output', 'inductive_val.txt')
#output_val_file_path = os.path.join('../../data/hugging_face_ratio_0.2/output', 'inductive_val.txt')
output_val_file_path = os.path.join('../../data/hugging_face_dummy10/output', 'inductive_val.txt')
...
#output_test_file_path = os.path.join('../../data/hugging_face/output', 'inductive_test.txt')
#output_test_file_path = os.path.join('../../data/hugging_face_ratio_0.2/output', 'inductive_test.txt')
output_test_file_path = os.path.join('../../data/hugging_face_dummy10/output', 'inductive_test.txt')
...
#~~~~~~~~~~~~~~~~
#output_train_data_maps_file_path = os.path.join('../../data/hugging_face/output','inductive_train_dict.txt')
#output_train_data_maps_file_path = os.path.join('../../data/hugging_face_ratio_0.2/output','inductive_train_dict.txt')
output_train_data_maps_file_path = os.path.join('../../data/hugging_face_dummy10/output','inductive_train_dict.txt')
...
#output_val_data_maps_file_path = os.path.join('../../data/hugging_face/output','inductive_val_dict.txt')
#output_val_data_maps_file_path = os.path.join('../../data/hugging_face_ratio_0.2/output','inductive_val_dict.txt')
output_val_data_maps_file_path = os.path.join('../../data/hugging_face_dummy10/output','inductive_val_dict.txt')
...
#output_test_data_maps_file_path = os.path.join('../../data/hugging_face/output','inductive_test_dict.txt')
#output_test_data_maps_file_path = os.path.join('../../data/hugging_face_ratio_0.2/output','inductive_test_dict.txt')
output_test_data_maps_file_path = os.path.join('../../data/hugging_face_dummy10/output','inductive_test_dict.txt')
...
sampled_test_edges_index = remaining_test_edges[0][0]
...
count = 0
sampled_val_edge_num =0

for i in range(remaining_val_edges.shape[0]):
    if remaining_val_edges[i][0] == sampled_val_edges_index and (sampled_val_edges_index in sampled_train_data_index)and count == 0:
        data_id = int(sampled_val_edges_index)
        key = data_id
        sampled_val_dict[key] = dict()
        sampled_val_dict[key]['models']=dict()
        sampled_val_dict[key]['performance'] = dict()
        temp_model_list = [int(remaining_val_edges[i][1])]
        temp_performance_list =[remaining_val_edges[i][2]]
        count+=1
        sampled_val_edge_num +=1
    elif remaining_val_edges[i][0] == sampled_val_edges_index and (sampled_val_edges_index in sampled_train_data_index) and count != 0:
        temp_model_list.append(int(remaining_val_edges[i][1]))
        temp_performance_list.append(remaining_val_edges[i][2])
        count += 1
        sampled_val_edge_num += 1
        if i==remaining_val_edges.shape[0]-1:
            sampled_val_dict[key]['models'] = np.array(temp_model_list)
            sampled_val_dict[key]['performance'] = np.array(temp_performance_list)
    elif remaining_val_edges[i][0] > sampled_val_edges_index and (remaining_val_edges[i][0] in sampled_train_data_index):
        sampled_val_dict[key]['models'] = np.array(temp_model_list)
        sampled_val_dict[key]['performance'] = np.array(temp_performance_list)
        sampled_val_edges_index = remaining_val_edges[i][0]
        count = 0
        data_id = int(sampled_val_edges_index)
        key = data_id
        sampled_val_dict[key] = dict()
        ...
        if i==remaining_val_edges.shape[0]-1:
            sampled_val_dict[key]['models'] = np.array(temp_model_list)
            sampled_val_dict[key]['performance'] = np.array(temp_performance_list)
    elif remaining_val_edges[i][0] > sampled_val_edges_index and (remaining_val_edges[i][0] not in sampled_train_data_index):
        continue

assert(sampled_val_edge_num==remaining_val_edges.shape[0])

count = 0
sampled_test_edge_num =0

for i in range(remaining_test_edges.shape[0]):
    if remaining_test_edges[i][0] == sampled_test_edges_index and (sampled_test_edges_index in sampled_train_data_index)and count == 0:
        data_id = int(sampled_test_edges_index)
        key = data_id
        sampled_test_dict[key] = dict()
        sampled_test_dict[key]['models']=dict()
        sampled_test_dict[key]['performance'] = dict()
        temp_model_list = [int(remaining_test_edges[i][1])]
        temp_performance_list =[remaining_test_edges[i][2]]
        count+=1
        sampled_test_edge_num +=1
    elif remaining_test_edges[i][0] == sampled_test_edges_index and (sampled_test_edges_index in sampled_train_data_index) and count != 0:
        temp_model_list.append(int(remaining_test_edges[i][1]))
        temp_performance_list.append(remaining_test_edges[i][2])
        count += 1
        sampled_test_edge_num += 1
        if i==remaining_test_edges.shape[0]-1:
            sampled_test_dict[key]['models'] = np.array(temp_model_list)
            sampled_test_dict[key]['performance'] = np.array(temp_performance_list)
    elif remaining_test_edges[i][0] > sampled_test_edges_index and (remaining_test_edges[i][0] in sampled_train_data_index):
        sampled_test_dict[key]['models'] = np.array(temp_model_list)
        sampled_test_dict[key]['performance'] = np.array(temp_performance_list)
        sampled_test_edges_index = remaining_test_edges[i][0]
        count = 0
        data_id = int(sampled_test_edges_index)
        key = data_id
        sampled_test_dict[key] = dict()
        ...
        if i==remaining_test_edges.shape[0]-1:
            sampled_test_dict[key]['models'] = np.array(temp_model_list)
            sampled_test_dict[key]['performance'] = np.array(temp_performance_list)
    elif remaining_test_edges[i][0] > sampled_test_edges_index and (remaining_test_edges[i][0] not in sampled_train_data_index):
        continue

assert(sampled_test_edge_num==remaining_test_edges.shape[0])
...
### saving the new data format: train_dict, val_dict, test_dict
#~~~~~~~~~~~~~~~~
#output_sampled_train_data_maps_file_path = os.path.join('../../data/hugging_face/output','initial_train_dict.txt')
#output_sampled_train_data_maps_file_path = os.path.join('../../data/hugging_face_ratio_0.2/output','initial_train_dict.txt')
output_sampled_train_data_maps_file_path = os.path.join('../../data/hugging_face_dummy10/output','initial_train_dict.txt')
...
#output_sampled_val_data_maps_file_path = os.path.join('../../data/hugging_face/output','initial_val_dict.txt')
#output_sampled_val_data_maps_file_path = os.path.join('../../data/hugging_face_ratio_0.2/output','initial_val_dict.txt')
output_sampled_val_data_maps_file_path = os.path.join('../../data/hugging_face_dummy10/output','initial_val_dict.txt')
...
#output_sampled_test_data_maps_file_path = os.path.join('../../data/hugging_face/output','initial_test_dict.txt')
#output_sampled_test_data_maps_file_path = os.path.join('../../data/hugging_face_ratio_0.2/output','initial_test_dict.txt')
output_sampled_test_data_maps_file_path = os.path.join('../../data/hugging_face_dummy10/output','initial_test_dict.txt')
...
##save train_edge_list , val_edge_list, test_edge_list
#output_train_file_path = os.path.join('../../data/hugging_face/output', 'initial_train.txt')
#output_train_file_path = os.path.join('../../data/hugging_face_ratio_0.2/output', 'initial_train.txt')
output_train_file_path = os.path.join('../../data/hugging_face_dummy10/output', 'initial_train.txt')
f=open(output_train_file_path, 'w')
for items in sampled_train_edge_list:
    ...
    f.write('\n')
f.close()
#output_val_file_path = os.path.join('../../data/hugging_face/output', 'initial_val.txt')
#output_val_file_path = os.path.join('../../data/hugging_face_ratio_0.2/output', 'initial_val.txt')
output_val_file_path = os.path.join('../../data/hugging_face_dummy10/output', 'initial_val.txt')
f=open(output_val_file_path, 'w')
...
#output_test_file_path = os.path.join('../../data/hugging_face/output', 'initial_test.txt')
#output_test_file_path = os.path.join('../../data/hugging_face_ratio_0.2/output', 'initial_test.txt')
output_test_file_path = os.path.join('../../data/hugging_face_dummy10/output', 'initial_test.txt')
f=open(output_test_file_path, 'w')
...
#test_ground_truth_file_path = os.path.join('../../data/hugging_face/output','test_ground_truth.csv')
#test_ground_truth_file_path = os.path.join('../../data/hugging_face_ratio_0.2/output','test_ground_truth.csv')
test_ground_truth_file_path = os.path.join('../../data/hugging_face_dummy10/output','test_ground_truth.csv')
...
#output_ground_truth_file_path = os.path.join('../../data/hugging_face/output','cutoff_ground_truth.csv')
#output_ground_truth_file_path = os.path.join('../../data/hugging_face_ratio_0.2/output','cutoff_ground_truth.csv')
output_ground_truth_file_path = os.path.join('../../data/hugging_face_dummy10/output','cutoff_ground_truth.csv')
...
#output_data_embed_file_path = os.path.join('../../data/hugging_face/output','data_embed.npy')
#output_data_embed_file_path = os.path.join('../../data/hugging_face_ratio_0.2/output','data_embed.npy')
output_data_embed_file_path = os.path.join('../../data/hugging_face_dummy10/output','data_embed.npy')
np.save(output_data_embed_file_path,updated_data_embed)

##save data_map
#output_data_map_file_path = os.path.join('../../data/hugging_face/output','data_map.txt')
#output_data_map_file_path = os.path.join('../../data/hugging_face_ratio_0.2/output','data_map.txt')
output_data_map_file_path = os.path.join('../../data/hugging_face_dummy10/output','data_map.txt')
...
##save model_map
#output_model_map_file_path = os.path.join('../../data/hugging_face/output','model_map.txt')
#output_model_map_file_path = os.path.join('../../data/hugging_face_ratio_0.2/output','model_map.txt')
output_model_map_file_path = os.path.join('../../data/hugging_face_dummy10/output','model_map.txt')
...
#output_model_embed_file_path = os.path.join('../../data/hugging_face/output','model_embed.npy')
#output_model_embed_file_path = os.path.join('../../data/hugging_face_ratio_0.2/output','model_embed.npy')
output_model_embed_file_path = os.path.join('../../data/hugging_face_dummy10/output','model_embed.npy')
np.save(output_model_embed_file_path,model_embed)

#save the running time of threshold processing
'''
with open(os.path.join('../../data/hugging_face/output', 'threshold_'+ str(threshold_score)+'_thresholdProcessing.txt'),'w')as f:
    f.write(str(threshold_time))
f.close()
'''
'''
with open(os.path.join('../../data/hugging_face_ratio_0.2/output', 'threshold_'+ str(threshold_score)+'_thresholdProcessing.txt'),'w')as f:
    f.write(str(threshold_time))
f.close()
'''
with open(os.path.join('../../data/hugging_face_dummy10/output', 'threshold_'+ str(threshold_score)+'_thresholdProcessing.txt'),'w')as f:
    f.write(str(threshold_time))
f.close()


#save the running time of probing processing
'''
with open(os.path.join('../../data/hugging_face/output', 'probing_budget_'+ str(total_budget)+'.txt'),'w')as f:
    f.write(str(sum(knn_process_time)))
f.close()
'''
'''
with open(os.path.join('../../data/hugging_face_ratio_0.2/output', 'probing_budget_'+ str(total_budget)+'.txt'),'w')as f:
    f.write(str(sum(knn_process_time)))
f.close()
'''
with open(os.path.join('../../data/hugging_face_dummy10/output', 'probing_budget_'+ str(total_budget)+'.txt'),'w')as f:
    f.write(str(sum(knn_process_time)))
f.close()
